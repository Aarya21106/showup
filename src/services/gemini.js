...
1. 💳 ONE payment activates an account: the ₹${config.depositAmountInr} refundable deposit. That's it — paying it unlocks Month 1 with FULL Pro access, completely free. There is no separate Basic/Pro fee to pay in Month 1, whether or not they picked one earlier.
...
6. 📋 Starting Month 2, the real subscription price applies: Basic ₹${config.pricing.basic.monthly}/month (reminders, check-ins, AI nutrition plan) or Pro ₹${config.pricing.pro.monthly}/month (+ diet logging, calorie tracking, burn logs, deep-dive coaching) — they choose which at renewal time. This price is reduced by their EARNED consistency discount from Month 1: ₹${config.weeklyDiscountInr} off per CLEAN (zero-miss) week, capped at ₹${config.maxDiscountInr}/month — never a flat one-time amount. Full consistency: Basic ₹${config.pricing.basic.minAfterDiscount}/month, Pro ₹${config.pricing.pro.minAfterDiscount}/month. This is a REAL discount applied to the actual renewal charge, not just a number shown in a message.
7. 🎟️ Promo Code: typed directly in chat (never an external page/form). A valid code unlocks 14 days of full Pro access free — no deposit either. Recorded deposit is set to ₹50 (waived, never charged) for internal tracking only. One-time use per account. Never guess, confirm, or reject a SPECIFIC code's validity yourself — that's decided elsewhere; just say you can't confirm it here.
8. ✅ Payment confirmation is fully automatic via webhook the moment real payment lands. NEVER tell a user to reply "paid" or type anything to confirm payment as a required step — that only exists as a background safety net if the automatic confirmation is ever delayed.
`;
...
1. FIRST, analyze the user's latest incoming message ("${message}") and extract/update all fields into "extracted":
   ...
   - "goal_timeframe": The user's own stated target timeframe for their goal, as free text (e.g. "3 months", "12 weeks", "6 months", "no rush"). Only set this from the user's reply to the Step 6.5 timeframe question below — do not infer it from anything else. Do NOT set this if their answer is implausibly short compared to the estimate you gave (see Step 6.5's validation rule) — leave it null and ask again instead.
...
3. Do NOT tell them to reply "paid" or type anything to confirm payment — say instead that their account activates automatically once payment is confirmed. If they haven't picked a tier yet, remind them to reply "1" or "2" to get their deposit link — that's the only payment needed to activate.`;
